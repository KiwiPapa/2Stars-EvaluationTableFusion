...
from TableFusionPro_UI import Ui_MainWindow
...
class Main_window(QMainWindow, Ui_MainWindow):
    ...
    def main_initialization(self):
        ...
        self.pushButton_11.clicked.connect(self.open_file3)
        self.pushButton_12.clicked.connect(self.open_file4)
        self.pushButton_13.clicked.connect(self.open_file5)
        self.pushButton_17.clicked.connect(self.open_file6)
        ...
        self.pushButton_53.clicked.connect(self.open_table_fusion_directory)  # 打开综合评价工区文件夹
    ...
    def open_file3(self):
        ...
        if fnames[0]:
            for fname in fnames[0]:
                self.lineEdit_49.setText(fname)

    def open_file4(self):
        fnames = QFileDialog.getOpenFileNames(self, '打开第二个文件', './')  # 注意这里返回值是元组
        if fnames[0]:
            for fname in fnames[0]:
                self.lineEdit_53.setText(fname)
    ...
    def open_file5(self):
        ...
        if fnames[0]:
            for fname in fnames[0]:
                self.lineEdit_50.setText(fname)

    def open_file6(self):
        fnames = QFileDialog.getOpenFileNames(self, '打开第二个文件', './')  # 注意这里返回值是元组
        if fnames[0]:
            for fname in fnames[0]:
                self.lineEdit_55.setText(fname)
    ##############################
    ...
    def table_process3(self):
        if self.lineEdit_49.text() != '' and self.lineEdit_53.text() == '':
            fileDir1 = self.lineEdit_49.text()
            self.xls_formatting_first_layer(fileDir1)
            QMessageBox.information(self, "提示", "一界面表格数据规范化完毕")
        elif self.lineEdit_53.text() != '' and self.lineEdit_49.text() == '':
            fileDir2 = self.lineEdit_53.text()
            self.xls_formatting_second_layer(fileDir2)
            QMessageBox.information(self, "提示", "二界面表格数据规范化完毕")
        elif self.lineEdit_53.text() != '' and self.lineEdit_49.text() != '':
            fileDir1 = self.lineEdit_49.text()
